...
from .node import Node, NodeCollection, NodeMetrics
...
"""
    ...
    def select_node(self) -> Node:
        """
        Select the most promising node using PUCT algorithm.
        ...
        best_node = None
        best_puct_score = -np.inf
        
        # Get all expandable nodes (nodes that can have children)
        expandable_nodes = [node for node in self.nodes 
                          if node.genealogy.generation < self.config.max_depth]
        
        if not expandable_nodes:
            # If no expandable nodes, select from all nodes
            expandable_nodes = list(self.nodes)
        
        total_visits = sum(node.visits for node in self.nodes)
        
        # Calculate PUCT scores for ranking
        node_scores = [(node.score, idx) for idx, node in enumerate(expandable_nodes)]
        node_scores.sort(reverse=True)  # Sort by score (higher is better)
        
        # Get adaptive C-PUCT value
        c_puct = self.get_adaptive_c_puct()
        
        for node in expandable_nodes:
            # Exploitation term: Use rank-based scoring
            rank = next(idx for score, idx in node_scores if expandable_nodes[idx] == node)
            rank_score = (len(expandable_nodes) - rank) / len(expandable_nodes) if len(expandable_nodes) > 1 else 1.0
            
            # Exploration term: UCB-style exploration (with adaptive C-PUCT)
            if total_visits > 0 and node.visits > 0:
                exploration_score = c_puct * math.sqrt(math.log(total_visits) / node.visits)
            else:
                exploration_score = float('inf')  # Prioritize unvisited nodes
            
            puct_score = rank_score + exploration_score
    ...
    def backpropagate(self, node: Node) -> None:
        ...
        while current is not None:
            current.add_visit(node.score)  # Use the leaf node's score as reward
